...
class Main extends React.Component {
  ...
  componentWillMount() {
    this._fetchMovies('discover');
  }

  _fetchMovies = (searchOrPopular, searchTerm) => {
    const fetchURL = (searchOrPopular === 'search' && searchTerm ) ?
      `http://api.themoviedb.org/3/${searchOrPopular}/movie?api_key=${config.api_key}&query=${searchTerm}` :
      `http://api.themoviedb.org/3/${searchOrPopular}/movie?api_key=${config.api_key}`;
  ...
  render () {
    ...
    return (
      <div className="Main">
        <input type="text" value={this.state.searchString} onChange={this.handleChange} placeholder="Search terms  here..."/>
